...
<div class="container">
  ...
  <div class="examples">
    ...
    <div class="example-buttons">
      {#each examples as example}
        <button class="example-btn" on:click={() => loadExample(example)}>
          {example.name}
        </button>
      {/each}
    </div>
  ...
  <div class="form-section">
    ...
    <div class="input-group">
      ...
      <textarea id="value" bind:value rows="8" placeholder="例: JSON 物件格式"
      ></textarea>
    </div>

    <div class="buttons">
      <button on:click={storeValue} disabled={isLoading} class="btn store">
        {isLoading ? "儲存中..." : "💾 儲存到 KV"}
      </button>

      <button on:click={getValue} disabled={isLoading} class="btn get">
        {isLoading ? "讀取中..." : "📖 從 KV 讀取"}
      </button>

      <button on:click={deleteValue} disabled={isLoading} class="btn delete">
